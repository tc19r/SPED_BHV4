...
#
#
# projs=project(X18,X13,E0)
# num_aux=np.array(projs).ndim
# # if num_aux==2:
# #     for ts in projs:
# #             traces.append([ts[0],ts[1],0,0])
# # else:
# traces.append([projs[0], projs[1], 0, 0])
# plot_trace(traces, mode='joint', labels=None,num_aux=num_aux,save_plot=False,title_add=None)
...
for id,edge in enumerate(edge_points):
    ...
    num_aux = 1
    traces.append([projs[0], projs[1], 0, 0])
    title_add=' Surface' +  str(id+1) +','+'Lambda='+str(projs[2])
