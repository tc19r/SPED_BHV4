#This code attempts to find the direct solution on the best surface,
#Treating it as a quadratic problem
#3/1/26, the code base is the 2/28's main. Did some cleaning.
#It has been observed on 2/28 that guaranteed the convergence for the original small data is hard, especially near the orign
#The reason is unclear, perhaps due to the small gradient near zero,  of the data, or just the embedding.
#Therefore, in this experiment, we magnify the data by 5 time first.
import numpy as np
...
def plot_surface(surf, books=None,flag_x_bar=0, flag_pgd_trace=0, M1=None, M2=None, M3=None,traces=None,flag_surf_one=1,flag_multi_traces=0,flag_embed=0,flag_plot_data=1,data_title='Uniform sampling in the \'blue\' Surface',quiver=None):
    ...
    if book and flag_plot_data==1:  #Note that this book here shouldn't be global - 2_2_26
        ...
        for i in range(data.shape[0]):
            ax_combined.scatter(data[i,0], data[i,1], data[i,2])
    ...
    if flag_pgd_trace:
        ...
        ax_combined.set_zlabel('Z')
    if quiver:
       ax_combined.quiver(0,0,0,quiver[0][0], quiver[0][1], quiver[0][2],color='b')
       ax_combined.quiver(0,0,0,quiver[1][0], quiver[1][1], quiver[1][2],color='b')
    plt.axis('equal')
    plt.show()
...
surfaces_data=[
    (Surface(X9,X17,t1,t2), 'brown', 'Surface 7'),
]
...
book={}
book['data']=z_t.T
# plot_surface(surfaces_data,books,traces=traces, flag_x_bar=0,flag_multi_traces=1,flag_embed=0,flag_pgd_trace=1,flag_plot_data=0,data_title='Sample trees from yeast data')
...
#3/1: trying solu
def project(X1=None,X2=None,E0=None):
    pass
...
# a=estar2
# b=1-estar2
# d_min= a*r1 + b*r2 + 2*np.sqrt(a*b)*r3
# c=np.sqrt(d_min)
# y=c*y
#
#
# book['data']=y[:, np.newaxis].T
# plot_surface(surfaces_data,books,traces=traces, flag_x_bar=0,flag_multi_traces=0,flag_embed=0,flag_pgd_trace=0,flag_plot_data=0,data_title='Orthogonal basis',quiver=basis)
#--------------------
